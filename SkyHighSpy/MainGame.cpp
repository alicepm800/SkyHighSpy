...
void UpdateAgent8() {
	...
	case STATE_APPEAR:
		...
		Play::DrawFontText("132px", "PRESS SPACE TO START LEVEL", { DISPLAY_WIDTH / 2, DISPLAY_HEIGHT / 2 }, Play::CENTRE);
	...
	case STATE_ATTACH:	
		...
		if (Play::KeyDown(VK_SPACE)) {
			...
			obj_agent8.rotation += PLAY_PI / 2.0f;
			Play::SetGameObjectDirection(obj_agent8, 4, obj_agent8.rotation );
			gameState.agentState = STATE_FLY;
...
void UpdateParticles() {
	...
	for (int particle_id : vParticles) {
		...
		Play::DrawObjectTransparent(particle, 0.08);
		int transparency = (lifetime - 1.0f) / -8.8f;
		if (transparency < 0) {
			transparency = 0;
		}
		Play::DrawObjectTransparent(particle, transparency);
		if (lifetime > 0.5f) {
